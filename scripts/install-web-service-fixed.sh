...
# WSLaragon Web Service Setup - FIXED VERSION
# Instala y activa el servicio web

set -e

echo "🌐 WSLaragon Web Service Setup (FIXED)"
echo "========================================="
...
print_status "Cleaning up old virtual environment..."
rm -rf venv
...
if systemctl is-active --quiet wslaragon-web; then
    ...
    if curl -s http://localhost:8080 > /dev/null 2>&1; then
        ...
        echo "⚠️ Service running but connection test failed"
        echo "Checking logs:"
        journalctl -u wslaragon-web --no-pager -n 10
    fi
else
