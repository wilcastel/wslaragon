...
| `site clone` | `[repository] [name]` <br> `--stack` / `--branch` / `--mysql` / `--database` / `--proxy` / `--env` / `--install` / `--import-db <sql>` / `--migrate` | Clona y configura dominio, HTTPS, Nginx, base de datos y proxy. Opcionalmente prepara `.env`, instala con Composer/pnpm, genera `APP_KEY`, importa MySQL y ejecuta migraciones explícitas. |
